...
def main(
    ...
    for image_path in (pbar := tqdm(image_paths, desc='Total images', disable=len(image_paths) <= 1)):
        ...
        if save_maps_:
            ...
            depth_min = panorama_depth[panorama_mask].min() if np.any(panorama_mask) else panorama_depth.min()
            depth_max = panorama_depth[panorama_mask].max() if np.any(panorama_mask) else panorama_depth.max()
            if depth_max > depth_min:
                norm_depth = (panorama_depth - depth_min) / (depth_max - depth_min)
            else:
                norm_depth = np.zeros_like(panorama_depth)
            depth_png = (norm_depth * 65535).astype(np.uint16)
            cv2.imwrite(str(save_path / 'depth.png'), depth_png)
